...
def initialize_sqlite_database(
    ...
    *,
    ...
    - Reject directory-as-database misuse
    - Deterministic parent directory creation
    - Idempotent schema application
    - No migrations or version branching
    """

    # ---------------------------------------------------------
    # Defensive canonicalization (composition root owns canonical source)
    # ---------------------------------------------------------
    canonical = Path(db_path).expanduser().resolve()

    # ---------------------------------------------------------
    # Guardrail: Reject directory paths
    # ---------------------------------------------------------
    if canonical.exists() and canonical.is_dir():
        raise ConfigurationError(
            f"SQLite database path points to a directory: {str(canonical)!r}"
        )
    ...
    # ---------------------------------------------------------
    # Use canonical connection factory
    # ---------------------------------------------------------
    conn = create_connection(canonical)
    try:
...
def _verify_required_pragmas(
    ...
    *,
    ...
    Important:
    - Must not set invariants (ownership belongs to create_connection()).
    - Fails fast for deterministic bootstrap behavior.
    """
