# tests/application/validation/test_enroll_student_in_course_validation.py

"""
Application-level validation tests for StudentManagementSystem.enroll_student_in_course
...
Phase: Relationship Command Validation (Enroll a Student in a Course)
"""
...
def test_enroll_student_missing_student_id_raises_missing_field_error(sms):
    with pytest.raises(MissingFieldError) as exc:
        sms.enroll_student_in_course(None, "C01")   # type: ignore[arg-type]

    assert exc.value.field == "student_id"
    assert exc.value.code == "missing_field"


def test_enroll_student_non_string_student_id_raises_invalid_type_error(sms):
    with pytest.raises(InvalidTypeError) as exc:
        sms.enroll_student_in_course(123, "C01")  # type: ignore[arg-type]

    assert exc.value.field == "student_id"
    assert exc.value.code == "invalid_type"


def test_enroll_student_whitespace_student_id_raises_invalid_identifier_error(sms):
    with pytest.raises(InvalidIdentifierError) as exc:
        sms.enroll_student_in_course(" ", "C01")
...
def test_enroll_student_missing_course_code_raises_missing_field_error(sms):
    with pytest.raises(MissingFieldError) as exc:
        sms.enroll_student_in_course("S01", None)    # type: ignore[arg-type]

    assert exc.value.field == "course_code"
    assert exc.value.code == "missing_field"


def test_enroll_student_non_string_course_code_raises_invalid_type_error(sms):
    with pytest.raises(InvalidTypeError) as exc:
        sms.enroll_student_in_course("S01", 123)    # type: ignore[arg-type]

    assert exc.value.field == "course_code"
    assert exc.value.code == "invalid_type"

def test_enroll_student_whitespace_course_code_raises_invalid_identifier_error(sms):
    with pytest.raises(InvalidIdentifierError) as exc:
        sms.enroll_student_in_course("S01", " ")
...
def test_enroll_student_validation_failure_has_no_side_effects(sms):
